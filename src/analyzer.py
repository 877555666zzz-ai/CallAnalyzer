...
def build_prompts(call: dict[str, Any], cfg: dict[str, Any]) -> tuple[str, str]:
    ...
    if roles_unknown:
        note = ("\n\nВНИМАНИЕ: роли в транскрипте НЕ размечены (моно-запись). Определи по смыслу, "
                "где говорит ОПЕРАТОР, а где КЛИЕНТ, и веди разбор соответственно.")
    else:
        # Роли размечены по каналам стерео-записи, но на реальных звонках канал иногда
        # даёт наводку/перекрёстную речь — метка у отдельной реплики может быть неверной.
        # Не доверяй ярлыку слепо, если он противоречит содержанию реплики.
        note = ("\n\nВНИМАНИЕ: роли размечены автоматически по каналам записи и МОГУТ ошибаться "
                "на отдельных репликах (наводка между каналами). Если содержание реплики явно "
